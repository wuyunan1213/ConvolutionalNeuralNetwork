...
def pooling_layer_forward(input, layer):
    ...
    for n in range(batch_size):
        input_n['data'] = input['data'][:, n]
        im = np.reshape(input_n['data'], (h_in, w_in, c))
        col = np.zeros((c,h_out*w_out))
        for i in range(c):
            for h in range(h_out):
                for w in range(w_out):
                    matrix_hw = im[h*stride: h*stride+k, w*stride: w*stride+k, i]
                    col[i, h*w_out + w] = np.amax(matrix_hw.flatten())
        output['data'][:, n] = col.flatten()
    # implementation ends
...
def pooling_layer_backward(output, input, layer):
    ...
    for n in range(batch_size):
        input_n['data'] = input['data'][:,n]
        im = np.reshape(input_n['data'],(h_in, w_in, c))
        col = np.zeros(im.shape)
        #grad = np.zeros((k*k*c, h_out*w_out))
        tmp_data_diff = np.reshape(output['diff'][:, n], (h_out,w_out, c))
        for i in range(c):
            for h in range(h_out):
                for w in range(w_out):
                    window = im[h*stride:h*stride+k,w*stride:w*stride+k,i]
                    window2 = np.reshape(window, (k*k))
                    window3 = np.zeros(window2.shape)
                    window3[np.argmax(window2)] = 1
                    col[h*stride:h*stride+k,w*stride:w*stride+k,i] = np.reshape(window3,(k,k)) * tmp_data_diff[h,w,i]
                    #mat_diff = mat_col*tmp_data_diff[h,w,i]
                    #grad[i*k*k:(i+1)*k*k,h*w_out+w] = mat_diff
        input_od[:,n] = col.flatten()
    return input_od
#############################################################################################
#########  YOU SHOULD MAINTAIN THE RETURN TYPE AND SHAPE AS PROVIDED IN STARTER CODE   ######
#############################################################################################
